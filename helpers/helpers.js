...
export function printTempUnit(unitState) {
    if (unitState == Units.metric)
        return '&deg;C';
    else
        return '&deg;F';
}
export function printSpeedUnit(unitState) {
    if (unitState == Units.metric)
        return 'km/h';
    else
        return 'mph';
}
export function parseUnitState(flag) {
    if (!flag)
        return Units.metric;
    else
...
export function secondsToHHMM(seconds) {
    if ((seconds >= 3600 || seconds <= -3600) &&
        ...
        throw Error('Number has to be more than 3600 and less than 90000 to maintain HH:MM format');
    }
}
