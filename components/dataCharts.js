...
//chart 1 ===================>
let chart1, chart2;
export async function renderChart(forecast, unitState) {
    const getMaxValueWithPadding = () => {
        return (Math.max(...forecast.map((row) => (row.rain ?? 0) + (row.snow ?? 0))) *
            1.1);
    };
    Chart.defaults.color = 'rgba(255, 255, 255, 0.8)';
    const chartCtr = document.querySelector('#temp-chart1');
    if (chart1)
        chart1.destroy();
    if (chart2)
        chart2.destroy();
    ...
    // Chart2 ===================>
    const chartCtr2 = document.querySelector('#temp-chart2');
...
//# sourceMappingURL=dataCharts.js.map