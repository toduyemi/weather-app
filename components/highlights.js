...
export function renderHighlights(current, unitState) {
    const template = getElement('#daily-statistics', HTMLTemplateElement);
    const highlights = document.createElement('ul');
    const highlightCard = document.importNode(template.content, true);
    highlightCard.querySelector('#sunrise').textContent = formatInTimeZone(current.highlights.sunrise, current.timezone, 'hh:mm aaaa');
    highlightCard.querySelector('#sunset').textContent = formatInTimeZone(current.highlights.sunset, current.timezone, 'hh:mm aaaa');
    ...
    highlightCard.querySelector('#beaufort').src =
        `./static/assets/weather-icons-master/production/line/all/wind-beaufort-${convertToBeaufort(current.highlights.wind_speed, unitState)}.svg`;
    // ternary condition to check unit state, if metric convert mps to kmh and then print unit
    highlightCard.querySelector('#windspeed').textContent = `${unitState === Units.metric
        ? mpsToKmh(current.highlights.wind_speed).toFixed(2)
        : current.highlights.wind_speed}${printSpeedUnit(unitState)} `;
    highlightCard.querySelector('#precipitation').textContent = `${(current.highlights.rain ?? 0) + (current.highlights.snow ?? 0)}mm/h `;
